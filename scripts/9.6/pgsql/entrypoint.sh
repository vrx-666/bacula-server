...
sed -i "s/Owner: bacula/Owner: $DB_User/g" /opt/bacula.db
sed -i "s/OWNER TO bacula/OWNER TO $DB_User/g" /opt/bacula.db
...
if [ $check_db -gt 0 ];then
	...
	if [ $check -eq 13 ] || [ $check -eq 14 ]; then
		...
		PGPASSWORD=${DB_Password} psql -h ${DB_Host} -p ${DB_Port} bacula ${DB_User} < /opt/bacula.db
		sleep 5s
	else
